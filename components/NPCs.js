window.renderNPCs = function(data, appInstance) {
    ...
    function renderNPCList(npclist) {
        ...
        return npclist.map(npc => `
            <div class="card npc-card" data-id="${npc.id}" style="margin-bottom:1rem; padding:1rem; cursor:pointer;">
    ...
    container.querySelector('#npc-search').addEventListener('input', () => {
        const query = container.querySelector('#npc-search').value.toLowerCase().trim();
        container.querySelectorAll('.npc-card').forEach(card => {
            const npc = data.npcs.find(n => n.id === card.dataset.id);
            if (!npc) return;
    ...
    function openNPCDetailModal(npc) {
            ...
            <div style="display:flex; gap:2rem;">
                ...
                <div style="flex:2;">
                    ...
                    <div style="background:rgba(139, 58, 58, 0.1); border:1px solid var(--danger); padding:1.5rem; border-radius:var(--radius-md);">
                        <h4 class="text-danger" style="margin-bottom:0.5rem; display:flex; align-items:center; gap:0.5rem;">📋 Notes</h4>
                        <p style="font-style:italic; color:var(--text-muted);">${npc.secret || 'No notes recorded.'}</p>
                    </div>
                </div>
            </div>
        `, (modalBody) => {
            modalBody.querySelector('#edit-npc-btn').addEventListener('click', () => {
                ...
                `, (editBody) => {
                    editBody.querySelector('#save-edit-npc-btn').addEventListener('click', () => {
                        ...
                        setTimeout(() => {
                            const card = document.querySelector(`.npc-card[data-id="${npc.id}"]`);
                            if (card) card.click();
                        }, 350);
                    });
                });
    ...
    // Add NPC
    container.querySelector('#add-npc-btn').addEventListener('click', () => {
        appInstance.openModal(`
            ...
            <label>Notes</label>
            <textarea id="new-npc-secret" rows="3" placeholder="Any notes about this NPC..."></textarea>
            <div style="text-align:right; margin-top:1rem;">
        ...
        `, (modalBody) => {
            modalBody.querySelector('#save-npc-btn').addEventListener('click', () => {
                ...
                const newNPC = {
                    ...
                    attitude: modalBody.querySelector('#new-npc-attitude').value,
